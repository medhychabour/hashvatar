// ─── gradient.js — rect + 6 blurred shapes, varied opacity/blend (diffuse gradient) ───
...
export function renderGradient(canvas, { size, colors, animated = false, seeds }) {
  ...
  function draw(phase) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      drawBlurredShape(off, SHAPES[i], size, layer.tx + driftX, layer.ty + driftY, rot, scale, hex, pad, pad);
      ctx.save();
      ctx.filter = `blur(${blur}px)`;
      ctx.globalCompositeOperation = opts.composite;
      ctx.globalAlpha = opts.alpha;
      ctx.drawImage(off.canvas, 0, 0, w, h, -pad, -pad, w, h);
      ctx.restore();
    }

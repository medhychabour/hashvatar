...
export function parseTone(tone) {
  const t = tone.trim();
  if (/^[a-zA-Z]+$/.test(t)) {
  ...
  if (t.startsWith('#') || /^[0-9a-f]{3,6}$/i.test(t)) {
    return rgbToOklch(...hexToRgb(t.startsWith('#') ? t : '#'+t));
  }
...
export function generateColor(seed, lSeed, cSeed, tones, isSecondary = false, baseHue) {
  let h, l, c;
  if (tones && tones.length > 0) {
    ...
    const hue = baseHue != null ? baseHue : seed * 360;
    h = (hue + 360) % 360;
    if (isSecondary) {
      l = 0.18 + cSeed * 0.20;
      c = 0.08 + lSeed * 0.12;
